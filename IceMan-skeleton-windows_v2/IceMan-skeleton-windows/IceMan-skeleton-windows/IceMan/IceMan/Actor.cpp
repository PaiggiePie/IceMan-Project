...
bool Agent::annoy(unsigned int amount) {
    cout << "Agent annoy" << endl;
    if (!isAlive())  { // Already dead
        cout << "is dead already" << endl;
        return false;
    }
    hitPoints -= amount;
...
void Boulder::doSomething() {
    ...
    if (fallingState == true) {
        if (sp->canActorMoveTo(this, getX(), getY() - 1) &&
            ...
            sp->canActorMoveTo(this, getX() + 3, getY() - 1)) {
            moveTo(getX(), getY() - 1);
            int t = sp->annoyAllNearbyActors(this, 500 , 3);
            if (t == 2) {
                sp->increaseScore(500);
            }
        }
        else {
            setVisible(false);
